...
### KITTI
def get_left_frames_kitti(kitti_root = '/home/data/kitti'):
...
def left_to_disparity_kitti(left_img_path, mode='disp_occ_0'):#'disp_occ_0'
...
### SceneFlow
#### Monkaa
def get_left_frames_monkaa(sceneflow_root = '/home/data/sceneflow'):
...
def left_to_right_monkaa(left_img_path):
...
def left_to_disparity_monkaa(left_img_path):
...
#### Driving
def get_left_frames_driving(sceneflow_root = '/home/data/sceneflow'):
        ...
        for subkey in os.listdir(os.path.join(sceneflow_root, subset, 'frames_finalpass', key)):
            for subsubkey in os.listdir(os.path.join(sceneflow_root, subset, 'frames_finalpass', key, subkey)):
                for img in os.listdir(os.path.join(sceneflow_root, subset, 'frames_finalpass', key, subkey, subsubkey, 'left')):
                    left_imgs_paths.append(os.path.join(sceneflow_root, subset, 'frames_finalpass', key, subkey, subsubkey,\
                                                        'left', img))
    return left_imgs_paths
      
def left_to_right_driving(left_img_path):
...
def left_to_disparity_driving(left_img_path):
...
#### FlyingThings3D
def get_left_frames_flyingthings3d(sceneflow_root = '/home/data/sceneflow', use_difficult_examples = False):
...
def left_to_right_flyingthings3d(left_img_path):
...
def left_to_disparity_flyingthings3d(left_img_path):
...
### Cityscapes
def get_left_frames_cityscapes(cityscapes_root = '/home/data/cityscapes', full_sequence = False):
    ...
    if full_sequence:
        subset = 'leftImg8bit_sequence'
    else:
        subset = 'leftImg8bit'
    left_images_path = os.path.join(cityscapes_root, subset)
    left_imgs_paths = []
...
def left_to_right_cityscapes(left_img_path):
...
def left_to_disparity_cityscapes(left_img_path):
...
def load_disparity_kitti(disparity_path):
    ...
    return torch.tensor(np.asarray(Image.open(disparity_path))).float().unsqueeze(0)/255.

### SceneFlow
def load_image_sceneflow(img_path):
    """Load the torch tensor containing an image from the SceneFlow dataset.
    
    The shape of the tensor is [3, 540, 960].
    """
    
    return torch.tensor(np.asarray(Image.open(img_path))).float().permute(2,0,1)[:3]/255.

def load_disparity_sceneflow(disparity_path):
    """Load the torch tensor containing a disparity map from the SceneFlow dataset.
    
    The shape of the tensor is [1, 540, 960].
    """
    
    return torch.tensor(np.flipud(loader.load_pfm(disparity_path)).copy()).float().unsqueeze(0)

### Cityscapes
def load_image_cityscapes(img_path):
...
def plot_history(trainer):
    ...
    ax.plot(trainer.history['train loss'], label='train loss', c='r', lw=3)
    ax.plot(trainer.history['valid loss'], label='valid loss', c='b', lw=3)
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    ax.legend()
...
# pretty print a dictionary, such as a wandb config
def pretty_print(d, indent=1):
